...
# State machine (each phase only consumes input from its own step):
#   IDLE → intent from user message
#   PHASE1_CONFIRM → "yes" confirms API only; never reads original_query for params
#   PHASE2_COLLECT / PHASE2_OPTIONAL → params from current message only
#   PHASE3_CONFIRM → "yes" executes; corrections from current message only
...
{extra}Respond with ONLY this JSON object — no markdown, no explanation outside JSON:
{{"api_id": "<id or null>", "confidence": "high|medium|low|none", "reason": "<one line>"}}"""
...
def chat():
    ...
    def reset():
        nonlocal state, current_api, raw_params, converted_params, original_query, last_param_source
        state = "IDLE"
        current_api = None
        raw_params = {}
        converted_params = {}
    ...
    def transition_to_phase3():
        ...
        converted, req_miss = apply_conversion_and_reconcile(
            api, raw_params, last_param_source
        )
    ...
    while True:
        ...
        if state == "IDLE":
            ...
            print("\n🔍 Identifying what you need...", flush=True)
        ...
        if state == "PHASE1_CONFIRM":
            ...
            if low in ("yes", "y", "yeah", "sure", "ok", "proceed", "go ahead", "correct"):
                ...
                # "yes" only confirms the API — do NOT scrape original_query for
                # parameters. Each phase collects input only in its own step.
                req_miss, opt_miss = get_missing_params(api, raw_params)

                if req_miss:
                    say(format_param_ask(req_miss, opt_miss, include_optional=False))
                    state = "PHASE2_COLLECT"
                elif opt_miss:
        ...
        if state == "PHASE2_OPTIONAL":
            ...
            if not extracted:
                _, opt_miss = get_missing_params(api, raw_params)
                say(
                    "I didn't catch a filter value in that message.\n"
                    "Try: `name SD_T` or `name to SD_T` or `type Database`\n\n"
                    + "\n".join(_format_param_line(p) for p in opt_miss)
        ...
        if state == "PHASE3_CONFIRM":
            ...
            # Parameter correction — only this message (not intent or prior phases).
            api = api_map[current_api]
            last_param_source = user_input
            extracted = phase2_extract_params(
                user_input, api, raw_params, phase="confirm_edit"
            )
            extracted = _drop_unmentioned_enums(api, extracted, user_input)
            if extracted:
                raw_params.update(extracted)
